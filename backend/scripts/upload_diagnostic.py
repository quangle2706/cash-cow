...
This uploads a diagnostic report file to S3 using boto3 SDK,
then creates a matching DiagnosticReport now (Day 3's async ORM setup)
pointing at the REAL S3 url - thus fulfilling problem statement's storage
architecture for the first time.

Run from backend/ with .venv active:
    python -m scripts.upload_diagnostic
"""
...
S3_KEY = "diagnostics/rx1001-002.txt" # upload to remote s3 bucket and create the file above and store it with the name rx1001-002.txt
...
async def record_diagnostic_report(file_url: str) -> str:
